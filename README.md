...
3. mod = '/path/to/caffe/models/bvlc_refference_detectnet/deploy.prototxt'
4. weights = '/path/to/digits/jobs/jobtitle/snapshot.caffemodel'
...
1. evaluation_and_analysis_functions is the function library containing model application and data analysis functions.  They are listed as follows:
...
To get necessary images for paper. We first need to get the coverage maps for the images.  certainty curves are generated by varying filter widths (for lowpass and highpass filters) or locations (for notch filters).  
